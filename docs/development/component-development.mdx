...
const definition: ComponentDefinition<Input, Output> = {
  ...
  async execute(input, context) {
    context.logger.info(`Fetching ${input.url}`);
...
const definition: ComponentDefinition<Input, Output> = {
  ...
  runner: {
    ...
    image: 'tool:latest',
    entrypoint: 'sh',
    command: ['-c', 'tool "$@"', '--'],  // Shell wrapper for PTY
    network: 'bridge'
  },
  inputSchema,
  outputSchema,
  async execute(input, context) {
    const args = ['-json', '-target', input.target];
...
async execute(input, context) {
  ...
  const file = await context.storage?.downloadFile(input.fileId);
...
Define UI metadata for the workflow builder:

```typescript
import { port } from '@shipsec/component-sdk';
...
const definition: ComponentDefinition<Input, Output> = {
  ...
  metadata: {
    ...
    icon: 'dns',
    author: { name: 'ShipSec', type: 'shipsecai' },
    inputs: [
      { id: 'domains', label: 'Domains', dataType: port.list(port.text()), required: true },
    ],
    outputs: [
      { id: 'results', label: 'Results', dataType: port.json() },
    ],
    parameters: [
      { id: 'threads', label: 'Threads', type: 'number', default: 10 },
      { id: 'recordTypes', label: 'Record Types', type: 'multi-select', options: [
        { label: 'A', value: 'A' },
...
const definition: ComponentDefinition<Input, Output> = {
  ...
  resolvePorts(params) {
    const dynamicInputs = (params.childRuntimeInputs || []).map(input => ({
      id: input.id,
...
async execute(input, context) {
  ...
  try {
    ...
    return await runComponentWithRunner(runnerConfig, parseOutput, input, context);
  } finally {
...
describe('my-component', () => {
  it('should process input correctly', async () => {
    ...
    const result = await component!.execute({ target: 'example.com' }, context);
